...
void main(void) {
    ...
    while(1){
        if (canal_ADC == 0){
            adc_start(canal_ADC);
            canal_ADC = 1;
            VOLTAJE1 = map(VALORPOT1, 0, 255, 0, 500);
            entero_POT = VOLTAJE1/100;
            DECIMALES1 = VOLTAJE1-entero_POT*100;
        }
        else if (canal_ADC == 1){
            adc_start(canal_ADC);
            canal_ADC = 0;
            VOLTAJE2 = map(VALORPOT2, 0, 255, 0, 500);
            entero_POT2 = VOLTAJE2/100;
            DECIMALES2 = VOLTAJE2-entero_POT2*100;
        }
        if(bandera == 1){
            Lcd_Set_Cursor(1,1);
            Lcd_Write_String("POTENCIOMETRO 1");
            Lcd_Set_Cursor(2,1);
            sprintf(s, "%d.%dV", entero_POT, DECIMALES1);
            Lcd_Set_Cursor(2,1);
            Lcd_Write_String(s);
        }
        else if(bandera == 0){
            ...
            Lcd_Set_Cursor(2,1);
            sprintf(s, "%d.%dV", entero_POT2, DECIMALES2);
            Lcd_Set_Cursor(2,1);
            Lcd_Write_String(s);
        }
